...
router.route('/add').post((req, res) => {
	...
	// console.log(email);
	if (email) {
		let linkSchedule = 'https://tduy.herokuapp.com/schedule-detail/' + newSchedule._id;
		let mailOptions = {
			from: process.env.USER_SENDMAIL || PARAMTERS.USER_SENDMAIL,
			to: account.person.email,
			subject: title,
			html: 'Hi, ' + account.person.name + '! Đây là thông tin hành trình của bạn: <p>Bắt đầu chuyến đi: <b>' + startDate.toString().substr(0, 10) + '</b><p> <p>Kết thúc chuyến đi: <b>' + endDate.toString().substr(0, 10) + '</b> <p> Các địa điểm khám phá:<b>' + hashtag.map(item => { return ' ' + item })
				+ `</b></p> <p> Xem chi tiết lịch trình chuyến đi của bạn tại: <link>${linkSchedule} </link></p>`
		};
